"""Frappe CTR benchmark (NFM mirror, libfm-encoded 10-field tabular)."""
...
@BENCHMARK_REGISTRY.register("frappe_ctr")
class FrappeCTRBenchmark(Benchmark):
    """Frappe CTR benchmark on the NFM-prepared mirror.

    Used by Wukong as one of the small public CTR datasets in its
    scaling-law evaluation. All 10 libfm fields are exposed; label is
    already binary (1.0 click vs 0.0 sampled negative).
    """
    ...
    def build(self) -> BenchmarkData:
        ...
        data_cfg.setdefault("name", "frappe")

        LOGGER.info("FrappeCTRBenchmark.build: instantiating datamodule")
