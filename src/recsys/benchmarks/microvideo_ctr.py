"""MicroVideo (THACIL) CTR benchmark."""
...
@BENCHMARK_REGISTRY.register("microvideo_ctr")
class MicroVideoCTRBenchmark(Benchmark):
    """MicroVideo (THACIL) CTR benchmark.

    Wukong's MicroVideo dataset, distributed via the THACIL release
    (10.9K users / 1.7M items / 12.7M interactions). Manual extract
    only — see [docs/datasets.md](../docs/datasets.md) for the layout.
    """
    ...
    def build(self) -> BenchmarkData:
        ...
        data_cfg.setdefault("name", "microvideo")

        LOGGER.info("MicroVideoCTRBenchmark.build: instantiating datamodule")
